...
Write `council-log/{{TOPIC_SLUG}}/round-{n}.md`:
...
Write `decision.md` with the structure below. **You MUST** normalize all user stories, acceptance criteria, and tests into these fixed schemas. Use stable IDs: `US-###`, `AC-US###-##`, `T-###`; each test lists **Related acceptance criteria** by criterion ID. If a field is unknown, write `TBD` with a one-line note.
