...
2. **Check for rejection**: if 2+ non-abstaining participants voted REJECT → proceed to **Step 2c** (Clarification via Telegram)
...
7. **If no consensus** → compose a **revised proposal** that explicitly addresses each objection, then proceed to **Step 2b** (Operator Round Confirmation) before broadcasting the next round
...
## Step 2b — Operator Round Confirmation via Telegram

After completing the synthesis for **every round where deliberation would continue** (no consensus yet, no 2+ REJECT, and round < maxRounds), you MUST ask the Council Operator for confirmation before broadcasting the next round.

> **Skip this step** when: consensus is reached (all APPROVE → go directly to Step 3), or 2+ REJECT (→ go to Step 2c for clarification), or the `ask_operator` tool is unavailable (→ proceed automatically to the next round).

### Procedure

1. **Compose a Telegram summary** of the round outcome. Use this format:
   ...
   Votes:
   • Product Analyst: {VOTE} — {one-line key point}
   • Architect: {VOTE} — {one-line key point}
   • QA Strategist: {VOTE} — {one-line key point}

   Consensus: No
   Key agreements: {brief}
   Outstanding objections: {brief}

   Proposed changes for Round {N+1}:
   {brief summary of revised proposal}

   Reply "continue" to proceed, "stop" to end deliberation, or add feedback to include in the next round.
   ```

2. **Call the `ask_operator` MCP tool** with the composed message. The tool sends the message to Telegram and **blocks** until the operator replies (timeout: 10 minutes).

3. **Read the operator's reply** and act accordingly:

   | Reply | Action |
   |-------|--------|
   | **"continue"**, **"ok"**, **"yes"**, **"go"**, **"proceed"** (case-insensitive) | Proceed to the next round with the revised proposal as planned |
   | **"stop"**, **"abort"**, **"end"** (case-insensitive) | Stop the deliberation. Write `escalation.md` using the escalation format in Step 3, noting "Operator requested stop after Round {N}" as the reason |
   | **TIMEOUT** or tool returns timeout error | Proceed automatically to the next round (operator assumed to be unavailable) |
   | **Any other text** | Treat as operator feedback. Incorporate the feedback into the revised proposal for the next round, adding an "Operator Feedback (via Telegram)" section to the revised proposal before broadcasting |

4. **If operator provides feedback**, augment the revised proposal:

   ```
   ## Revised Proposal — Round {N+1}

   ### Operator Feedback (via Telegram)
   - {Operator's message, verbatim or summarized if very long}

   ### Changes from previous round
   - [What changed and why, referencing specific objections]
   - [Additional changes based on operator feedback, if applicable]

   ### Current proposal
   [The updated proposal incorporating feedback from both the team and the operator]

   ### Open questions
   [Anything that needs specific input from a particular role]
   ```

> **Note**: The `ask_operator` tool is provided by the `telegram-ask` MCP server (registered in `.mcp.json`). If the tool is unavailable (MCP server not configured or env vars missing), skip this step entirely and proceed to the next round automatically. Log a note in the round file: "⚠️ Operator confirmation skipped — ask_operator tool unavailable."

---

## Step 2c — Clarification via Telegram (on REJECT)
...
- **Actor**: [primary user or system role, e.g. API consumer, operator, service]
...
[Key architectural decisions, as proposed by Architect and validated by the council. Reference bounded contexts, services, schemas, messaging, and files where useful. Link to user story IDs when a decision is scoped to a story, e.g. `(see US-001)`.]

## Tests

[One block per test case. IDs like `T-001`, `T-002`, … Cover unit, integration, contract, or e2e as agreed; **Type** must name the layer explicitly.]

### T-001

- **Test ID**: T-001
- **Scenario**: [Given/When/Then style or clear single scenario description]
- **Type**: [e.g. unit | integration | contract | e2e | load — pick what applies]
...
### On Rejection (2+ REJECT votes — after Step 2c fails or is unavailable)

This section applies only if Step 2c was attempted and the operator replied "abort", the tool timed out, or the `ask_operator` tool was unavailable. Write `council-log/{{TOPIC_SLUG}}/rejection.md` with:
...
- **Operator confirmation**: after each non-consensus round, you MUST execute **Step 2b** (Operator Round Confirmation) to ask the operator via Telegram whether to continue. Only skip if the tool is unavailable (proceed automatically) or if the round ended in consensus/rejection.
- **Rejection duty**: if 2+ participants vote REJECT, do NOT attempt to interpret the ambiguity or push the team to choose an interpretation. First attempt **Step 2c** (Clarification via Telegram) to ask the operator. Only write `rejection.md` if the operator replies "abort", the tool times out, or the `ask_operator` tool is unavailable. The council must not guess user intent.
